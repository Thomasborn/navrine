...
export default function Blog() {
    ...
    const translatedArticles = t('blog.articles', { returnObjects: true }) as any[];

    if (!mounted || !Array.isArray(translatedArticles)) return <div className="min-h-screen bg-black" />;

    const articles = [
        {
            id: "scaling-nextjs",
            title: translatedArticles[0]?.title || "Scaling Next.js for Enterprise Data Solutions",
            excerpt: translatedArticles[0]?.excerpt || "How we architected a high-throughput financial dashboard handling millions of rows using server actions and edge caching.",
            category: translatedArticles[0]?.category || "Engineering",
            date: "2026-02-28",
        ...
        },
        {
            ...
            title: translatedArticles[1]?.title || "The Intersection of AI and Brand Identity",
            excerpt: translatedArticles[1]?.excerpt || "Why static logos are dead. Exploring dynamic, algorithmic visual systems that adapt to user behavior.",
            category: translatedArticles[1]?.category || "Design",
            date: "2026-02-15",
        ...
        },
        {
            ...
            title: translatedArticles[2]?.title || "Optimizing Your Platform for AI Agents",
            excerpt: translatedArticles[2]?.excerpt || "Traditional SEO is shifting. How to structure your data so GPTBot and Perplexity actually recommend your product.",
            category: translatedArticles[2]?.category || "Strategy",
            date: "2026-03-01",
        ...
        },
        {
            ...
            title: translatedArticles[3]?.title || "Deploying Custom LLM Infrastructure",
            excerpt: translatedArticles[3]?.excerpt || "A technical deep dive into fine-tuning open-source models for highly specific corporate knowledge bases.",
            category: translatedArticles[3]?.category || "AI",
            date: "2026-01-22",
        ...
        <main className="min-h-screen bg-[#050505] text-white selection:bg-[#bdfb54] selection:text-black font-sans pt-32 px-6 md:px-12 lg:px-24" aria-label="Navrine Insights and Technical Blog">
            ...
            <div className="max-w-7xl mx-auto mb-32">
                <header className="mb-20">
                    ...
                    >
                        {t('blog.title')} <br />
                        <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#bdfb54] to-[#bdfb54]/40">{t('blog.subtitle')}</span>
                    </motion.h1>
                    <motion.p
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                ...
                <section className="mb-24" aria-label="Featured Article">
                    ...
                    >
                        <Image
                            ...
                            fill
                            sizes="100vw"
                            className="object-cover transition-transform duration-1000 group-hover:scale-105 opacity-60 mix-blend-luminosity group-hover:mix-blend-normal z-0"
                        />
                        ...
                        <div className="relative z-20 w-full max-w-4xl">
                            ...
                            <div className="flex items-center gap-2 text-white font-bold uppercase tracking-widest text-sm group-hover:gap-4 transition-all">
                                {t('blog.readArticle')} <ArrowRight className="w-5 h-5 text-[#bdfb54]" aria-hidden="true" />
                            </div>
                        </div>
                    </motion.article>
                </section>
                ...
                <section aria-label="Latest Articles">
                    <div className="flex items-center justify-between border-b border-white/10 pb-6 mb-12">
                        <h2 className="text-2xl font-bold tracking-tight">{t('blog.latest')}</h2>
                    </div>
